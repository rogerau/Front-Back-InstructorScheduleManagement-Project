...
<head>
     <!--
         ...
         Filename: queryDevMode.php
   
      -->

    <title> Display all the courses and sections taught in a particular delivery mode </title>
...
/* Display all the courses and sections taught in a particular delivery mode
...
try {
    ...
    $sql= "SELECT CourseId, SectionId, DeliveryMode FROM Course_Section WHERE DeliveryMode = :deliverymode";  
    ...
    if($row) {
        ...
        echo '<table>';
        echo '<tr> <th>CourseId</th> <th>SectionId</th>  <th>DeliveryMode</th> </tr>';
        do {
            echo '<tr><td>' . $row['CourseId'] . '</td><td>' . $row['SectionId'] . '</td><td>' . $row['DeliveryMode'] . '</td></tr>';
        }
